...
/*
 * EarthArmor currently broken due to PKListener.
 * When a user dies the leather armor should not drop.
 */
public class EarthArmor extends EarthAbility {
	...
	public EarthArmor(Player player) {
		super(player);
		if (CoreAbility.hasAbility(player, EarthArmor.class) || bPlayer.isOnCooldown(this)) {
			return;
		}
	...
	private boolean moveBlocks() {
		...
		if (headBlock.getLocation().distanceSquared(player.getEyeLocation()) > range * range || legsBlock.getLocation().distanceSquared(player.getLocation()) > range * range) {
			remove();
			return false;
		}
